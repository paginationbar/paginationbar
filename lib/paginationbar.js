...
class x {
  constructor(e) {
    ...
    h(t, n.containerClassName), h(
      t,
      `${n.themeClassNamePrefix}${this.options.theme}`
    ), this.options.disabled && h(t, "disabled"), this.render();
  }
  emitCurrentPageChange(e) {
    var t;
    (t = this.options) == null || t.onCurrentPageChange(e);
  }
  emitPageSizeChange(e) {
    var t;
    (t = this.options) == null || t.onPageSizeChange(e);
  }
  setCurrentPage(e, t = !0) {
    let i = Number.parseInt(e.toFixed(0));
    return i < this.options.firstPageNumber ? i = this.options.firstPageNumber : i > this.lastPageNumber && (i = this.lastPageNumber), i !== this.options.currentPage && (this.options.currentPage = i, this.emitCurrentPageChange(i), t && this.render()), i;
  }
  setPageSize(e, t = !0) {
    let i = Number.parseInt(e.toFixed(0));
    return i > this.options.total && (i = this.options.total), i !== this.options.pageSize && (this.selectedPageSize = i, this.options.pageSize = i, this.emitPageSizeChange(i), t && this.render()), i;
  }
  setTotal(e, t = !0) {
    e !== this.options.total && (this.options.total = e || 0, t && this.render());
  }
  setOptions(e, t = !1) {
  ...
  setTheme(e) {
    ...
    for (const i of t.classList.values())
      i.startsWith(n.themeClassNamePrefix) && u(t, i);
    h(t, `${n.themeClassNamePrefix}${e}`);
  }
  disabled(e, t = !0) {
    const i = this.getContainerEl();
    this.options.disabled = e, e ? h(i, "disabled") : u(i, "disabled"), t && this.render();
  }
  get pageCount() {
  ...
  getContainerEl() {
    if (typeof this.options.container == "string") {
      ...
      if (!e)
        throw new Error(c.getContainerElError);
      return e;
    }
  ...
  generatePager() {
    const e = this.finalPager.reduce((t, i) => {
      const r = `
        <span class="pager-quick-btn-icon pager-quick-btn-icon--more ${i.type}">${v}</span>
        <span class="pager-quick-btn-icon pager-quick-btn-icon--arrow ${i.type}">${f}</span>
      `, a = this.isPagerNumberType(i.type) ? i.pageNumber : r, g = this.options.currentPage === i.pageNumber ? "active" : "", o = this.isPagerNumberType(i.type) ? "pager-number" : "pager-quick-btn";
      return t += `<li class="${n.pagerItemClassName} ${g} ${o}" 
        data-number="${i.pageNumber}" data-type="${i.type}" role="${o}">
        ${a}
      </li>`, t;
    }, "");
  ...
  generatePrev() {
    const e = this.options.currentPage <= this.options.firstPageNumber || this.options.disabled ? 'disabled="disabled"' : "", t = e ? "disabled" : "", i = this.options.prevText ? this.options.prevText : b;
    return `<button type="button" class="${n.prevButtonClassName} ${t} ${this.isBackground()}" ${e} role="prev-btn">${i}</button>`;
  }
  generateNext() {
    const e = this.options.currentPage >= this.lastPageNumber || this.options.disabled ? 'disabled="disabled"' : "", t = e ? "disabled" : "", i = this.options.nextText ? this.options.nextText : P;
  ...
  pagerListener(e) {
    ...
    if (t.getAttribute("role") === "pager-number") {
      const r = this.getPagerItemDataset(t), a = Number(r.number);
